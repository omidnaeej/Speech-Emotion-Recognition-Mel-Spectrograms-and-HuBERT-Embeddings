...
FILENAME_RE = re.compile(r"^(?P<spk>\d{4})_.*_(?P<emo>[A-Z]{3})_.*\.wav$")
...
def stratified_split(paths: List[str], labels: List[int], val_size: float, test_size: float, seed: int):
    ...
    print(f"Stratified split: {len(paths)} samples")
...
def build_dataloaders(cfg: dict):
    ...
    for p in all_wavs:
        ...
        if len(seen_spk) >= cfg["speaker_count"]:
            pass
    print(f"Selected {len(selected)} files from {len(seen_spk)} speakers")
